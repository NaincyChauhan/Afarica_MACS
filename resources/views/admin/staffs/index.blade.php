@extends('layouts.admin') @section('title') Staffs @endsection 
@section('css')
<!-- DataTables -->
...
@section('js')
    ...
    <script src="{{ asset('app-assets/masterX/js/staffs.js') }}"></script>
@endsection 
@section('content')
            ...
            <div class="card-body">
                <div class="row" id="headerRow">
                    ...
                    <a class="btn btn-primary btn-rounded btn-fw float-right text-white" data-toggle="modal"
                        data-target="#addUser">
                        <i class="mdi mdi-plus-circle-outline"></i> Staff
                    </a>
                </div>
                        ...
                        <tbody>
                            @foreach($staffs as $user)
                                <tr>
                                    ...
                                    <td>{{ $user->user->roles()->exists() ? $user->user->roles()->first()->name : '' }}</td>
                                    <td>{{ $user->name }}</td>
                                    <td>{{ $user->email }}</td>                                            
                                    <td>{{ $user->mobile }}</td>                                            
                                    <td>
                                        <label class="switch">
                                            <input type="checkbox" onchange="changeStatus($(this))" href="{{ route('change-staff-status', $user->user->id) }}" {{ $user->user->status == 1 ? 'checked' : '' }}>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            @can('update-user')
                                                <a class="btn btn-success btn-sm btn-icon-text mr-3 text-white" onclick="event.preventDefault(); callUpdate($(this))" href="{{ route('staff.edit', $user->id) }}">
                                                    <i class="mdi mdi-table-edit"></i> Edit
                                                </a>
                                            @endcan
                                            @can('delete-user')
                                                <a class="btn btn-danger btn-sm btn-icon-text" onclick="event.preventDefault(); callDelete($(this))" href="{{ route('staff.destroy', $user->id) }}">
...
<div class="modal fade" id="addUser" >
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('staff.store') }}" id="request-form-add" enctype="multipart/form-data">
